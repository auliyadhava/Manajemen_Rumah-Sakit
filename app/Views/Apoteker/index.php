                                    ...
                                    <tr class="hover:bg-gray-700/30 transition-colors group">
                                        ...
                                        <td class="px-6 py-5">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="text-xs text-gray-400">Riwayat Sakit: <?= esc($r['diagnosis']) ?></p>
                                                    <button class="btn-detail text-xs text-blue-400 hover:text-blue-300"
                                        ...
                                        <td>
                                            <?php if ($r['payment_status'] == 'paid'): ?>
                                                <span class="inline-flex items-center gap-1.5 bg-green-500/10 text-green-400 px-3 py-1 rounded-full text-xs font-bold border border-green-500/20">Lunas</span>
                                            <?php else: ?>
                                                <span class="inline-flex items-center gap-1.5 bg-red-500/10 text-red-400 px-3 py-1 rounded-full text-xs font-bold border border-red-500/20">Belum Lunas</span>
                                            <?php endif; ?>
                                        </td>
...
<div id="detailModal" class="hidden fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
    <div class="bg-gray-800 w-full max-w-lg rounded-xl p-6 relative">
        ...
        <h3 class="text-lg font-semibold mb-4 text-white">Detail Resep</h3>
        <p class="text-sm text-gray-400 mb-3">Pasien: <span id="modalPatient" class="text-white"></span></p>
    ...
    btn.addEventListener('click', function() {
        ...
        fetch("<?= base_url('apoteker/detail/') ?>" + id)
            ...
            .then(data => {
                ...
                if(data.length > 0){
                    data.forEach(item => {
                        html += `<div class="flex justify-between bg-gray-700 p-2 rounded">
                                    <span>${item.name}</span>
                                    <span>Qty: ${item.quantity}</span>
                                 </div>`;
                    });
                } else {
                    html = '<p class="text-gray-400">Belum ada obat untuk resep ini.</p>';
                }
                modalContent.innerHTML = html;
            })
            .catch(err => {
                modalContent.innerHTML = '<p class="text-red-500">Gagal memuat detail obat!</p>';
                console.error(err);
            });
    });
});
